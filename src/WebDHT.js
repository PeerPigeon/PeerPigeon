...
export class WebDHT extends EventEmitter {
        ...
        if (this.localStorage.has(keyId)) {
            ...
            if (!this.isExpired(storedData)) {
                ...
                const responseData = {
                    keyId,
                    found: true,
                    messageId: messageId,  // Echo back the EXACT messageId from request
                    key: storedData.key,
    ...
    async handleValueChangedNotification(fromPeerId, data) {
        ...
        let hasSubscribers = this.mySubscriptions.has(keyId);
        let hasLocalSubscribers = this.subscriptions.has(keyId) && this.subscriptions.get(keyId).size > 0;
