...
// ── SHA-1 peer ID generation (PSP spec §5.1) ─────────────────────────────────
...
const INSTALL_SEED_KEY = 'peerpigeon_install_peer_seed';
function getOrCreateInstallPeerSeed() {
  let seed = localStorage.getItem(INSTALL_SEED_KEY);
  if (!seed) {
    seed = randomHex(20);
...
function getOrCreateTabId() {
  if (!_tabSeed) _tabSeed = randomHex(8);
  return _tabSeed;
}

async function generatePeerId() {
  const installSeed = getOrCreateInstallPeerSeed();
  const tabId = getOrCreateTabId();
  return sha1Hex(installSeed + ':' + tabId);
}

export const customPeerId = ref('');
generatePeerId().then(id => { customPeerId.value = id; });

// ─────────────────────────────────────────────────────────────────────────────

// Shared DHT instance across all pages
export const dht = ref(null);
export const gossip = ref(null);
...
watch(dht, (newDht) => {
  if (newDht && newDht.on) {
    ...
    newDht.on('signaling:connected', (data) => {
      nodeId.value = newDht.getClientId() || data?.clientId || '';
    ...
    newDht.on('peer:connected', async (peerId) => {
      const isHexPeerId = peerId && /^[0-9a-f]{16,128}$/.test(peerId);
      
      if (isHexPeerId) {
        const uuidsToRemove = peerList.value.filter(id => id.includes('-') && id.length < 40);
        peerList.value = peerList.value.filter(id => !uuidsToRemove.includes(id));
